...
export default function LoginForm() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (signInError) {
      setSubmitError(authErrorMessage(signInError, "Gagal masuk. Coba lagi."));
      return;
    }
    ...
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-background px-4">
      ...
      <div className="w-full max-w-sm rounded-2xl border border-border bg-card p-8 shadow-xs ring-1 ring-slate-950/5">
        <div className="mb-6 text-center">
          ...
          <h1 className="mt-3 text-2xl font-bold tracking-tight text-foreground">Masuk</h1>
          <p className="mt-1 text-sm text-muted-foreground">Masuk untuk melanjutkan ke akun Anda.</p>
        </div>
