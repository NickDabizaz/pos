...
export async function GET(request: Request) {
  ...
  const rows = await findBarisLaporanPenjualan(db, { dari, sampai, termasukDibatalkan, idlokasi });
  const namaLokasi = await resolveNamaLokasi(db, idlokasi);
  const konteks = buatKonteksLaporanPenjualan(namaPerusahaan, { dari, sampai, namaLokasi }, new Date());
